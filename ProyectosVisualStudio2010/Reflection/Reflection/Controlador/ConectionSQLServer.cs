...
namespace Reflection.Controlador
{
    class ConectionSQLServer
    {
        ...
        public Reflection.Modelo.Estudiante CreateQuerySelectEstudiante(string sFiltro)
        {
            Reflection.Modelo.Estudiante et = new Modelo.Estudiante();
        ...
        public Reflection.Modelo.Profesor CreateQuerySelectProfesor(string sFiltro)
        {
            Reflection.Modelo.Profesor pf = new Modelo.Profesor();
            sqlString = string.Format("SELECT * FROM [Example].[dbo].[{1}] WHERE (ID_Persona = {0})", sFiltro, "Persona");
            CreateQueryReader(sqlString);
            if (QueryReader.Read())
            {
