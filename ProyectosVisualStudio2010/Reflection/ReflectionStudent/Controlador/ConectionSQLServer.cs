...
namespace ReflectionStudent.Controlador
{
    class ConectionSQLServer
    {
        ...
        public ReflectionStudent.Modelo.Estudiante CreateQuerySelectEstudiante(string sFiltro)
        {
            ReflectionStudent.Modelo.Estudiante et = new Modelo.Estudiante();
        ...
        public ReflectionStudent.Modelo.Profesor CreateQuerySelectProfesor(string sFiltro)
        {
            ReflectionStudent.Modelo.Profesor pf = new Modelo.Profesor();
            sqlString = string.Format("SELECT * FROM [Example].[dbo].[{1}] WHERE (ID_Persona = {0})", sFiltro, "Persona");
            CreateQueryReader(sqlString);
            if (QueryReader.Read())
            {
